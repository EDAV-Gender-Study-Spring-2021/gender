...
```{r}
df <- data_q3 %>% 
  select(Country.Name,Country.Code, Indicator.Code, c(40:64)) %>%
  group_by(Country.Name, Country.Code, Indicator.Code)%>%
  summarise_if(is.numeric, mean, na.rm = TRUE)
```
...
```{r}
df <- dcast(df, Country.Name + Country.Code ~ Indicator.Code)
...
```{r}
dfj<-df[!(df$Country.Name=="Japan" | df$Country.Name == "Nigeria"),]
```
