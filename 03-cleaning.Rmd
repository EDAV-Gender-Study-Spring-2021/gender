...
For social rights, the dataset has a lot of questions related to the women social rights with binary labels attached. We basically divided the 24 rights related questions into 6 topics: 2 for Finance, 3 for Freedom, 8 for Law, 3 for Marriage, 4 for Job and 4 for Welfare. If the answer to the question is yes, then label 1. Otherwise, label 0. Therefore, we average the binary number in each topic and get the averaged score for each topic. Then the problem we discussed to analyze the social rights becomes to analyze the distribution of the scores for each countries. (The details are in the check_list.csv)
  
For education, we selected some indicators that represent the female and male education status across the time like school enrollment and rate of graduation. Moreover, in order to detect the correlation between female education status and other factors, we also selected GDP, fertility rate,  female rural population and government expenditure on education department. To reduce the effect of missing values, we use average scores across multiple years instead of data from specific year on most of the plot except for the time series plots.
